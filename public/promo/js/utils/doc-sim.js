 ...
 * @role 15s Deep PDF 3-Stage Map-Reduce & Fast-Pass Document Intelligence Simulation
 */

(function () {
  const steps = [
    { title: 'Stage 1: Fast-Pass High-Speed Tokenization', desc: 'PDF 128페이지 텍스트 및 구조 클라이언트 메모리 매핑 중...', pct: 28 },
    { title: 'Stage 2: WebGPU Parallel Cluster Map-Reduce', desc: '16개 청크 병렬 텐서 요약 연산 진행 중 (0 byte 외부 전송)...', pct: 68 },
    { title: 'Stage 3: Hierarchical Knowledge Deck Synthesis', desc: '4대 도메인(비즈니스, 기술, 보안, 재무) 지능형 카드 덱 완성!', pct: 100 }
  ];
  ...
  window.runDocMapReduceSim = function () {
    ...
    statusText.innerHTML = `<strong>${steps[0].title}</strong>: ${steps[0].desc}`;
    ...
    const interval = setInterval(() => {
      ...
      if (currentStep < steps.length) {
        ...
        statusText.innerHTML = `<span style="color:#10b981; font-weight:700;"><i class="fas fa-check-circle"></i> 128페이지 3단계 맵리듀스 완료 (소요시간: 2.8초, 100% 로컬 연산)</span>`;
        btn.disabled = false;
        btn.innerHTML = `<i class="fas fa-rotate-left"></i> 다시 시뮬레이션`;

        // Render Summary Deck Cards
        deckContainer.style.display = 'grid';
        deckContainer.innerHTML = `
          <div class="deck-card" style="animation: fadeInUp 0.35s ease;">
          ...
          <div class="deck-card" style="animation: fadeInUp 0.45s ease;">
          ...
          <div class="deck-card" style="animation: fadeInUp 0.55s ease;">
          ...
          <div class="deck-card" style="animation: fadeInUp 0.65s ease;">
