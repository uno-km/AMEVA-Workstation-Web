...
export function AIStatusIndicator({ 
  ...
  // 추상화된 2초 자동 소멸 비간섭 미니 말풍선 훅 연동
  const { isVisible: isAiToastVisible } = useConditionToast(
    isMainReady,
    'AI 로딩 완료!',
    { variant: 'emerald', icon: <Sparkles size={11} color="#34d399" />, durationMs: 2000 }
  );
  ...
  return (
    <div style={{ position: 'relative' }} ref={dashboardRef}>
      ...
      <UnobtrusiveToastBubble
        show={isAiToastVisible}
        message="AI 로딩 완료!"
        icon={<Sparkles size={11} color="#34d399" />}
        variant="emerald"
        placement="top"
        offset={8}
      />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
              ...
              {isGhostLoading && (
                <div style={{ marginTop: '6px' }}>
                  ...
                  <div style={{ fontSize: '9px', color: '#67e8f9', textAlign: 'left', wordBreak: 'break-all', marginTop: '2px' }}>
                    {ghostProgressText}
                  </div>
                </div>
              )}
          ...
          <div style={{ marginTop: '2px' }}>
            {!isReady && !isLoading ? (
              <button 
                ...
                style={{ width: '100%', padding: '10px', background: 'linear-gradient(135deg, #2563eb 0%, #06b6d4 100%)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontWeight: 600, boxShadow: '0 4px 12px rgba(37,99,235,0.3)', transition: 'transform 0.1s' }}
